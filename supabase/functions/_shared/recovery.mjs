...
export async function fetchComments({ token, mediaId, graphVersion, maxPages = 200 }) {
  const config = makeConfig(token, graphVersion);
  try {
    return await graphGetAll(`${mediaId}/comments`, { fields: COMMENT_FIELDS, limit: 50 }, config, config.token, maxPages);
  } catch (err) {
    if (!(err instanceof GraphError) || err.code !== 100) throw err;
    return graphGetAll(`${mediaId}/comments`, { fields: COMMENT_FIELDS_FALLBACK, limit: 50 }, config, config.token, maxPages);
  }
}
